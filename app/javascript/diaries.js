...
const diariesIndexJS = () => {
  ...
  const renderNewFoodsMealForm = (data) => {
    ...
    forms.forEach((form) => {
      ...
      if (
        form.id === "foods-meal-form" &&
        form.dataset.formType === undefined
      ) {
  ...
  const updateNewFoodsMealHTML = (form, foods, meal, foodsInMeal) => {
    ...
    foods.forEach((food) => {
      ...
      const mealId = foodForm.querySelector(
        "input[name='foods_meal[meal_id]']"
      );
      const foodId = foodForm.querySelector(
        "input[name='foods_meal[food_id]']"
      );
  ...
  const updatedMealItemHTML = (food, foodsMealId) => {
    ...
    const mealItem = mealItemTemplate
      .querySelector(".meal-item")
      .cloneNode(true);
  ...
  const onClickRemoveFood = (foodsMealId) => {
    // grab 'remove food'
    const removeFoodButton = document.querySelector(
      `[data-id='${foodsMealId}']`
    );
